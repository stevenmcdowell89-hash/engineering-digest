...
- **Readability over polish (v1.36 — a nudge, not a rule).** The digest can carry dense technical material because the sentences carrying it are easy to read. If the reader is fighting with sentences — long compound chains, stacked clauses, meta-references to the issue itself, scenario painting, a knowing tone — the prose is getting in the way of the substance. Default to describing what happened in clean declarative sentences. A useful self-test: read a paragraph aloud; if a sentence is past roughly 30 words or three clauses with a parenthetical, break it. A Friday-morning briefing, not a literary essay — it doesn't need to be Tolstoy. This is editorial calibration, not a compliance-checklist item; no structural test, no count.
...
**Coverage is layered by design (v1.37 — the recall layer).** Named and beat-framed queries are the *precision* layer: they are finite, and anything a list or a framing did not anticipate is invisible to them. Three standing rules supply recall. No single layer is trusted to be complete.

1. **Name-agnostic patterned catch-alls — run every issue, alongside the framed sweeps.** Shape queries, not name queries: *"<sector> company layoffs restructure <month>"* · *"engineering team restructure <month>"* · *"payments company workforce cut <month>"* · *"bank technology reorganisation <month>"* · *"tech company cuts engineering roles <month>"*, plus the equivalents for whichever concern is being swept. These catch organisations no list anticipated. **A sweep that has gone name-specific has stopped being a sweep** — once a query names a company it is drilling into a known story, and the catch-all must still run separately.

2. **Cross-beat entity sweep.** Any organisation that surfaces in one operational concern is swept once against the *other three concerns' framings* before Phase 1 closes — explicitly including **workforce, restructure, layoffs, reorganisation and team-shape** terms, which are the framings most likely to sit outside the beat where the company first appeared. A company found under a loyalty framing is queried again under a management framing, and vice versa. Stories that span concerns are the most valuable in the pool and the easiest to miss, because each beat's framing sees only its own slice.

3. **Record the catch-alls in `queries_run`.** Both layers appear in the artefact, so Phase 2 and Phase 5 can see which ran. A concern whose `queries_run` contains only name-specific and single-framing queries has not been swept.
...
- **News-event Lead test (v1.32):** the Lead must be driven by a news event from the past 7 days (or meet the new-in-window traction exception). A *news event* is: a launch, a vendor decision, a corporate restructure, a security incident with narrative weight, an outage with leadership implications, a research publication, a peer-org engineering decision, a payments-rail shift, a major hire/exit/restructure, a regulatory action that passes the scope-conditional bar. *"Marc Brooker published a blog post this week"* is **not** a news event — the publication is the event but the substance is a reflective essay, which belongs in Leadership Read / In Practice / Outside In. The Issue 12 v1.31 failure repeated this: Brooker as the Lead read as analysis, not news. v1.32 makes the Lead test news-first.
- **Anchor balance (v1.32):** of the 3 anchor slots (Lead, QT1, QT2), **at least 2 must be news events.** The third can be analysis/commentary if it's the week's strongest non-event story. This puts news up front (the first 60% of the magazine) and analysis where it belongs (Leadership Read / In Practice / Outside In at the back, where the slow reads close the issue).
- **AI-substrate ceiling — reader-experience count (v1.32, supersedes v3.8 anchor-only count):** count AI as substrate across the **6 featured sections** — Lead, QT1, QT2, Editorial Moment, Outside In, In Practice. **Cap at 4 of 6.** The v3.8 anchor-only count (≤2 of 3 anchors) failed in Issue 12 v1.31 — anchors passed at 1 of 3, but the reader experienced an AI-saturated issue because Editorial Moment, Outside In, and In Practice were all AI-driven too. At least 2 of the 6 featured sections should have AI as **not** the substantive driver. If the only strong material this week is genuinely AI-driven everywhere, the run plan documents the call and names what non-AI candidates were considered and rejected.
...
- **Within-issue concern diversity (v1.31):** Of the 3 anchor slots (Lead, QT1, QT2), **at most ONE can come from a single operational concern.** Lead + QT1 + QT2 must span at least 2 concerns; spanning 3 is preferred.
- **Adjacent-source rule (v1.32):** **no two consecutive featured sections (Lead → QT1, QT1 → QT2, QT2 → In Practice, In Practice → Other News, Outside In → Leadership Read, etc.) can have the same source or outlet.** Issue 12 v1.31 had Stripe SPT as QT2 immediately followed by Stripe Engineering's *"You can't whisper at an AI agent"* as In Practice — two Stripe pieces back-to-back even though the source-diversity-per-issue cap (2 sections) wasn't exceeded. The cap catches *count*; v1.32 catches *adjacency*. If two strong same-source candidates land in the run plan, one moves to a non-adjacent slot (Other News, On the Radar) or is swapped for a different-source piece.
- **Long-form blog variety rule (v1.35 — Standard Weekly only):** the three long-form reflective sections (*In Practice*, Outside In, The Leadership Read) must collectively cover **at least two different aspects of engineering leadership**. **At most ONE of the three can have AI / agents as its primary subject.** Issue 12 v1.32 shipped with all three long-form pieces on coding agents (In Practice = Atlassian / Wu on 16% / agent-era practice; Outside In = Shopify / natural-language spec layer for agentic workflows; Leadership Read = Brooker / coding agents as feedback loops). Even though the v1.32 reader-experience AI count (≤4 of 6 featured) passed, the *back-of-book reading experience* was a three-essay AI cluster. v1.35 closes the gap: when In Practice is AI-substrate, the other two long-form sections cover non-AI engineering leadership — management craft (1:1s, performance reviews, calibration, hiring loops), technical leadership (architectural decisions, technical strategy when not the SME, managing upwards), culture and ways of working (incident culture, blameless culture, async rituals), org structure (team topologies, manager-of-managers, IC-vs-management track), production / operating at scale (post-mortem culture, on-call design), career & coaching (growth conversations, retention). **Deep Dives are exempt** — by design, a deep dive's structure converges all sections on the single topic owning the issue.
...
**Whole-source rule (v1.37).** When Phase 3 fetches a primary source to verify something — an earnings transcript, a filing, a status page, a post-mortem, a press release — it reads that source for **all material news it contains**, not only the figure it was sent to pin. Anything material that is not already in `candidates.json` is reported back to the main loop as a late find, which routes to Phase 2 for a slot decision. Phase 3 is the last point in the pipeline with primary sources open; a fact visible there and not surfaced is a fact the issue ships without. Worked failure: at Issue 20, Phase 3 fetched the Visa Q3 FY2026 transcript to confirm value-added-services revenue and read it for nothing else, while a 7% workforce cut announced the same day went unrecorded and had to be added post-publication.
...
### Four operational concerns (v1.32 — Banking/fintech/loyalty repositioned as sub-theme)

The digest covers software engineering broadly for engineering leadership. **AI is the substrate that runs through all four concerns — not a fifth concern.** Tool releases are not first-order subject matter; the operational decisions they trigger are.

1. **Management & teams** — the everyday craft of running engineering managers and the teams they run: 1:1s, performance conversations, calibration and levelling, career frameworks, hiring loops and interview design, structured onboarding, the manager-of-managers transition, skip-levels, coaching and feedback culture, retention, growth plans. Frame for engineering managers with 1–3 years of experience, including those from non-technical routes. When AI shows up, it's because rollouts are changing what managers manage.

2. **Ways of working** — how teams deliver, organise, adopt, and shift practice. Delivery metrics (DORA, SPACE), trunk-based dev rollouts, CI/CD adoption, internal developer platforms, observability strategy as practice, incident-response culture, team topologies. The agentic-coding rollout question lives here. Vendor decisions belong here when the angle is *what teams do differently* — not the tool's own release notes.

3. **Operating at scale** — running production systems and the engineering organisations that keep them running. Post-mortems with narrative weight, outage analysis, on-call design, disaster recovery, observability under load. Dependency lifecycle when upgrade work is real and the deadline is live. Security stories when active exploitation, mass impact, or live compliance deadlines are present.

4. **Wider engineering landscape (v1.32 — replaces v1.31 Banking, fintech & loyalty as the fourth concern)** — cross-industry engineering news, vendor moves with cascade effects, major hyperscaler decisions (launches, restructures, outages), landmark research findings, significant labour-market shifts, major technical / engineering trends, regulatory shifts affecting tech broadly, peer engineering decisions at scale-tech orgs (Netflix, Spotify, Atlassian, GitHub, Mozilla, Cloudflare, etc.). **If a story would be the biggest engineering news at Spotify, GitHub, or Mozilla as much as at the reader's org, it lives here.** Hyperscaler events (AWS / GCP / Azure decisions, outages, pricing changes) belong here as first-class material when scale or cascade warrants — they are NOT demoted to reference-only as in v1.30. A major OpenAI / Anthropic / Google corporate decision with cross-industry engineering implications belongs here.

### Banking, fintech & loyalty as a cross-cutting sub-theme (v1.32 — repositioned from primary concern)

The reader is at a fintech serving UK and ME banks as customers, in the customer-loyalty / commerce-media space. Bank engineering content, fintech moves, and loyalty / commerce-media tech are a **consistent sub-theme** that surfaces under whichever primary concern fits:

- A Monzo data-mesh piece is *Ways of working* with a banking-engineering surfacing note
- A Stripe SPT payments-rail shift is *Wider engineering landscape* (cross-industry cascade) with a loyalty/commerce-media surfacing note
- A Wise calibration redesign is *Management & teams* with a fintech-peer surfacing note
- A Visa Trusted Agent Protocol launch is *Wider engineering landscape* (cross-industry agent commerce) with a loyalty surfacing note

**Phase 1 loyalty research emphasis (v1.27 retained):** Phase 1 still runs explicit weekly queries for loyalty / commerce-media engineering (Stripe Ledger, neobank loyalty rollouts, agentic-commerce protocol shifts, aggregator API moves, rewards-platform engineering). When a story surfaces it takes a normal slot under its primary concern. **Wider engineering news is not crowded out** — if the choice is between a strong non-fintech piece and a forced fintech piece, the non-fintech piece wins.

**Bank-regulator decisions (v1.32 — scope-conditional, replaces v1.31 blanket block):** Bank-regulator stories (FCA, ADGM/FSRA, PSR, HMT) are eligible for anchor slots only when they have **substantive engineering implications for the broader industry** — the test is *"would this matter at Mozilla, Spotify, or GitHub?"* If yes, eligible. If no (story hits a specific regulated entity class with no broader cascade), At a Glance / On the Radar / Other News only. FCA PS25/12 fails the test (UK payment institutions specifically). A hypothetical EU AI Act enforcement decision creating engineering requirements across every AI-using tech org passes. The v1.31 blanket block was overcorrection; the v1.32 test restores the right gate.
...
9. **Palette by concern (v1.32 — guardrail tightened)** — Management & teams = orange (`.section-label.orange`), Ways of working = teal (default `.section-label`), **Operating at scale = teal (default) BY DEFAULT — `.section-label.alert` (red) ONLY when the story is active-threat or active-upgrade-window** (Mini Shai-Hulud, Cisco SD-WAN active exploitation, a regulatory deadline this week). A Brooker-style reflective piece on Operating at scale uses default teal, not red. The Wider engineering landscape concern uses default teal. Outside In = orange (existing). **Structural landmarks always use the default teal regardless of content concern** — the Week at a Glance section label, the Foreword label, the In Practice header label, etc. are navigational signposts and don't carry concern-palette cues. Issue 12 v1.31 misapplied red to both the WAG label and the Brooker Lead label; v1.32 makes the rule explicit.
...
11. **In Practice** (v1.32 — first long-form section, interleaved) — standalone craft section. Full-width `.in-practice-header` (off-white, deep-teal top + left borders, v1.31) flowing into the off-white `.in-practice` inset body. Appears in roughly 9 of every 10 issues; omitted only when source material is genuinely thin. See § In Practice for full rules.
12. **Other News This Week** — one sentence + source link per item. 3–6 items. **News between long-form sections** (v1.32 — breaks up the back-of-book essay cluster).
13. **Platform Updates / What's worth knowing this week** (v3.2 — condensed) — **0 to 2 rows MAX**. Include ONLY items where impact is widespread right now OR the audience needs to make a near-term decision. Most weeks one row or omitted entirely. When rows are present: three columns (Status / Platform / What Changed), colour-coded status dots, tighter padding.
14. `<hr class="feature-break">`
15. **Outside In** (v1.32 — second long-form section, repositioned to here) — uses the format menu. **Default placement is at position 15**, between Platform Updates and On the Radar. Format C (compact inset) can place earlier after the Lead. **Format B (hero) is retired in v1.32** — it explicitly placed before the Leadership Read, which violated the new no-adjacent rule.
16. **On the Radar** (v1.32 — moved forward from position 17) — **timeline treatment** with a vertical connector. Deadlines / required actions first, then events. **News-adjacent content placed between Outside In and Leadership Read** (v1.32 — separates the two long-form sections).
17. `<hr class="feature-break">`
18. **The Leadership Read** (v1.32 — third long-form section, the slow close) — navy `.mgmt-section`. The reflective close to the issue. Only when source material earns it; omit if nothing strong. Max two consecutive issues without one.
19. **Recommended Reading** — linked list.
20. **Footer** — includes an **archive bar** below the next-edition line. The archive bar shows the last 5 issues by date and format type as pill-shaped badges (`.archive-pill`), with the current issue highlighted in teal (`.archive-pill.current`).
...
The rest of the year rotates through other craft themes (3–4 issues each). **Theme pool (expanded v1.35):**

*Management craft:* 1:1s and skip-levels · hiring loops & interview design · new-manager survival · feedback culture · coaching & growth conversations · manager-of-managers transitions · retention conversations · career frameworks & ladder design · onboarding new hires · async rituals & running good staff meetings.

*Technical leadership for managers (added v1.35):* making technical decisions when you're not the SME · navigating architectural reviews as a manager · build-vs-buy decisions as a leadership conversation · technical strategy under uncertainty.

*Managing across and upwards (added v1.35):* managing upwards / influencing senior leadership · escalation craft · presenting engineering work to non-technical exec audiences · cross-team partnership and negotiation · handling reorgs and ambiguous mandates.

*Agentic / AI-era management:* agent-era engineering practice · calibration and levelling under agent-assisted output · role-shape changes as AI lands · the disappearing junior pipeline conversation.

The seasons rotate. **Don't let any single theme dominate the run** — including agent-era practice. The v1.35 long-form-variety rule pushes against multi-week clusters where the In Practice + Outside In + Leadership Read all converge on AI.
...
Choose the format that best fits the source material — no cross-issue rotation tracker. **v1.32 default placement: position 15** (between Platform Updates and On the Radar — interleaved with news to avoid back-of-book essay cluster). Format C may place earlier after the Lead when the source is brief.

**Format A: Full feature** (default) — off-white background section (`.outside-in-section`), h3 subheadings, `.oi-callout` for the core lesson, stats row if data warrants, bold "The takeaway" line. Placement: position 15 (between Platform Updates and On the Radar).

**Format B: Hero moment — RETIRED in v1.32.** The `.oi-hero-band` treatment placed Outside In immediately before the Leadership Read, which violated the v1.32 no-adjacent-long-form rule. The CSS class `.oi-hero-band` remains in the template for reference but is not used in current issues. If a source has a single striking admission that speaks for itself, use Format A with a prominent pull-quote bar inside the section.
...
**Omit** — no Outside In this issue. Use when nothing earns inclusion. The format menu is "include in one of two shapes (A or C), or skip" — don't force content to fill the slot.
