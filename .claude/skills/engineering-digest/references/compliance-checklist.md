...
- [ ] **Phase 1 recall layer ran (v1.37)**: `queries_run` for each concern shows **name-agnostic patterned catch-alls** (shape queries — *"<sector> company layoffs restructure <month>"*, *"engineering team restructure <month>"*, etc.) alongside the named and beat-framed queries. A concern whose `queries_run` contains only name-specific queries, or only one framing, has **not** been swept — fail and send back to Phase 1. Watch specifically for the name-anchoring pattern: a generic opener, two hits, then every later query naming those two companies
- [ ] **Phase 1 cross-beat entity sweep (v1.37)**: organisations surfacing in one concern were swept against the other concerns' framings — explicitly including workforce / restructure / layoffs / reorganisation terms. Spot-check two companies that appear in the pool under a single framing and confirm the cross-framing query ran. This is the check that would have caught the Issue 20 Visa miss, where `Visa` and `layoffs` never co-occurred in any query
- [ ] **Phase 3 whole-source rule (v1.37)**: where Phase 3 fetched a primary source to verify a figure (earnings transcript, filing, status page, post-mortem), `section-briefs.md` records that the source was read for all material news, and any late find was reported back rather than left unrecorded
...
- [ ] **Four operational concerns (v1.32)**: the concerns are Management & teams / Ways of working / Operating at scale / **Wider engineering landscape** (cross-industry news, vendor moves, hyperscaler decisions, landmark research, peer eng-org moves at Spotify/Mozilla/GitHub/Cloudflare scale). The v1.31 "Banking, fintech & loyalty" as a primary concern is retired — it's now a cross-cutting sub-theme.
- [ ] **News-event Lead test (v1.32)**: the Lead is a news event from the past 7 days — a launch, vendor decision, corporate restructure, security incident with narrative weight, outage with leadership implications, research publication, peer-org engineering decision, payments-rail shift, hire/exit/restructure. **A blog post / reflective essay is NOT a Lead** — those go to Leadership Read / In Practice / Outside In. Issue 12 v1.31 failed this: the Lead was Brooker's blog post, which is essay-substrate, not news. The lead frames *what happened this week* in the first sentence.
- [ ] **Anchor balance (v1.32)**: ≥2 of 3 anchors (Lead, QT1, QT2) are news events. The third can be analysis/commentary if it's the week's strongest non-event story. Use the `event_or_essay` field in candidates.json to enforce.
- [ ] **Tool releases are not leads (v1.31 retained)**: Cursor / Copilot / Claude Code feature ships are At a Glance unless they trigger an operating-model, budget, or org-shape conversation. The lead's takeaway is never "the tool can now do X" — it's "as a leader, the conversation is now Y"
- [ ] **AI substrate classification (v3.4)**: AI stories are classified by the operational decision they trigger, not as a standalone beat
- [ ] **AI-substrate ceiling — reader-experience count (v1.32, supersedes v3.8 anchor-only count)**: across the 6 featured sections (Lead, QT1, QT2, Editorial Moment, Outside In, In Practice), no more than 4 may have AI as the substantive driver. At least 2 of the 6 featured sections have AI as not the primary force. Issue 12 v1.31 passed the v3.8 anchor-only check (1 of 3 AI anchors) but failed the reader experience (6 of 6 featured sections AI-flavoured); v1.32 catches this.
- [ ] **Soft breadth guardrail (v3.4)**: across rolling 4 issues, leads span at least 3 of the 4 operational concerns
- [ ] **No-repeat-featured-anchor (v1.31)**: no story that was a featured anchor (Lead, QT1, or QT2) in either of the previous 2 issues appears as a featured anchor in this issue, even in a different slot
- [ ] **Within-issue concern diversity (v1.31)**: Lead + QT1 + QT2 span at least 2 operational concerns; at most ONE anchor per concern
- [ ] **Adjacent-source rule (v1.32)**: no two consecutive featured sections (Lead → QT1, QT1 → QT2, QT2 → In Practice, In Practice → Other News, Outside In → Leadership Read, etc.) have the same source or outlet. Issue 12 v1.31 had Stripe QT2 immediately followed by Stripe In Practice — passed the per-issue source-diversity cap (2 sections) but failed adjacency. v1.32 catches adjacency.
- [ ] **Long-form blog variety (v1.35 — Standard Weekly only)**: the three long-form reflective sections (*In Practice*, Outside In, The Leadership Read) collectively cover **at least two different aspects of engineering leadership**. **At most ONE of the three has AI / agents as its primary subject** — the other two cover non-AI engineering leadership (management craft, technical leadership, culture / ways of working, org structure, production / operating at scale, career & coaching). Issue 12 v1.32 shipped all three long-form pieces on coding agents (Atlassian / Wu, Shopify, Brooker) — the v1.32 reader-experience AI count (≤4 of 6) passed but the back-of-book read was a three-essay AI cluster. v1.35 closes that gap. Deep Dives are exempt — they converge by design.
- [ ] **Banking, fintech & loyalty as sub-theme (v1.32 — repositioned from primary concern)**: bank engineering content, fintech moves, payments-rail engineering, and loyalty / commerce-media tech surface under whichever primary concern fits, with a surfacing note. They are NOT a separate concern with slot quota. The Phase 1 loyalty research emphasis (v1.27) is retained — loyalty queries run every week — but slot allocation is by substance, not patch-relevance
- [ ] **Bank-regulator scope-conditional test (v1.32 — replaces v1.31 blanket block)**: bank-regulator stories (FCA / ADGM / FSRA / PSR / HMT) are eligible for anchor slots only when implications cross-industry — the test is *"would this matter at Mozilla, Spotify, or GitHub?"* If yes, eligible. If no (story hits a specific regulated entity class with no broader cascade), At a Glance / On the Radar / Other News only. FCA PS25/12 fails the test (UK payment institutions specifically). EU AI Act enforcement passes (cross-industry).
...
- [ ] **No week count in the theme label (v1.34 — re-enforces v3.5 rule that slipped in Issue 12 v1.32 test run)**: the `.ip-theme` italic label shows the theme name only (*"Performance conversations"*, *"Hiring loops"*, *"Agent-era engineering practice"*). It does NOT contain a week count (*"week 1 of 3"*, *"week 2 of 4"*, *"part 1 of 3"*) or any progress marker. The reason: seasons are cut short when material thins (per the "cut the season short rather than drag" rule below). A visible week count would turn a clean season-close into a broken promise; the v3.5 rule keeps the label silent on the count so the editorial decision is invisible to the reader. **Grep check**: search the rendered `.ip-theme` element for *"week"*, *"of 3"*, *"of 4"*, *"part "*, *"#1"*, *"#2"* — each must return 0.
...
- [ ] **Section spread (v1.32 — hardened from v3.8 "at least one mid-issue" to "no two adjacent")**: no two long-form sections (*In Practice*, Outside In, The Leadership Read) are adjacent in the rendered issue. Each must be separated from the next by a news or news-adjacent section (Other News, Platform Updates, On the Radar). The v1.32 default order: News block (Lead + QTs) → IP → Other News → Platform Updates → OI → Radar → LR. If the rendered HTML has any two of the three long-form sections directly adjacent, the structure has failed and Phase 4 must be sent back to re-order. **Outside In Format B (`.oi-hero-band`) is retired in v1.32** — it always placed before Leadership Read and therefore always violated the no-adjacent rule.
...
- [ ] **Palette by concern (v1.32 — guardrail tightened)**: Management & teams = orange, Ways of working = default teal, **Operating at scale = default teal BY DEFAULT** — `.section-label.alert` (red) is used ONLY when the story is active-threat or active-upgrade-window (Mini Shai-Hulud, Cisco SD-WAN active exploitation, an active deadline this week). A reflective Operating-at-scale piece (Brooker-style) uses default teal, not red. Wider engineering landscape concern uses default teal. Outside In stays orange. **Structural landmarks (Week at a Glance label, Foreword label, In Practice header label, Editorial Moment label) always use the default teal regardless of content concern** — they are navigational signposts, not content cues. Issue 12 v1.31 misapplied red to both the WAG label and the Brooker Lead label; v1.32 catches this.
