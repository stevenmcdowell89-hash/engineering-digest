...
- **v1.38 (July 2026)** — Page limit retired, word budget adopted. The Standard Weekly's *"5–7 pages (hard limit)"* had been breached by **ten consecutive issues** — issues 11–20 rendered at 9–13 pages and not one hit 7 — because it was arithmetically unreachable against the section substance floors the spec itself mandates. Lead 300 + QT1 150 + QT2 150 + *In Practice* 300 (explicitly not a page-cut lever) + Leadership Read 400 is 1,300 words before the Foreword, Week at a Glance, Editorial Moment, Other News, Outside In, On the Radar, Recommended Reading, takeaway lines or source attributions; a full issue with every section at its floor lands near 2,600 words. Walking the spec's own cut order at Issue 20 freed only ~529 words and still left ten pages, with the next cut blocked by the ≥2-anchor-news-event floor. A rule every issue breaks is not a rule — it is a line Phase 5 writes an excuse around each week, which is exactly what Issue 20's review had to do. **Standard Weekly is now 2,600–3,700 words of body copy; Light Edition 1,200–2,000; Deep Dive unbounded.** The range is set from the measured spread of issues 11–20 (2,463–3,728 words, median ~3,160), not chosen. **The floor is enforced as well as the ceiling** — below 2,600 the issue is a Light Edition that has not admitted it, which ties the budget to the existing *"when in doubt, go Light"* rule. **Page count becomes a derived observation, not a constraint** (~250–300 words/page at current typography, so ~10–13 pages); nothing is cut to hit a page number. Page-count enforcement is renamed word-budget enforcement with the same cut order and the same Leadership Read 400-word floor. Compliance checklist and the SKILL.md quick-reference table updated to match.
- **v1.37 (July 2026)** — Recall layer for research. Issue 20 shipped without Visa's 28 July announcement of a 7% workforce cut (~2,600 roles, concentrated in technology and product, AI cited by the CEO, product and development teams reorganised from ~10 people into groups of 2–4). The story never entered `candidates.json` at all — it was not surfaced and rejected, it was never found — and it had to be promoted to the Lead post-publication. Two Phase 1 patterns combined to hide it. **(1) Name-anchoring:** the *Management & teams* layoff sweep opened generically, hit Monday.com and Intel, then went name-specific for every subsequent query — it stopped sweeping and started drilling. **(2) Beat siloing:** the *Banking/fintech* sweep ran sixteen queries including two on the card networks, but framed every one on loyalty, commerce media or earnings, so `Visa` and `layoffs` never co-occurred in a single query. The story fell in the seam between two beats and neither beat's framing could see it. v1.37 adds three rules, mirroring the fix `product-digest` made at its v1.4: **name-agnostic patterned catch-alls** as a standing recall layer run every issue; a **cross-beat entity sweep** — any organisation surfacing in one concern is swept once against the other three concerns' framings, with workforce/restructure named explicitly; and a **Phase 3 whole-source rule** — a primary source fetched for verification (earnings transcript, filing, status page, post-mortem) is read for all material news in it, not only the figures being pinned. Phase 3 had the Visa Q3 transcript open and read it for VAS revenue alone. Compliance checklist gains a matching evidence item.
- **v1.36 (May 2026)** — Readability nudge. **Not a rule, not a compliance-checklist item — explicitly a calibration note.** Added one paragraph to Audience & Tone in editorial-spec.md: the digest can carry dense technical material because the sentences carrying it are easy to read; if the reader is fighting with sentences (long compound chains, stacked clauses, meta-references to the issue itself, scenario painting, a knowing tone), the prose is getting in the way. Default to describing in clean declarative sentences. Read-aloud self-test: sentences past ~30 words or three clauses with a parenthetical get broken. A Friday-morning briefing, not a literary essay — doesn't need to be Tolstoy. Triggered by two Issue 11 passages (Outside In opener and takeaway) where the prose drifted into dramatising rather than describing — no structural rule was violated, just calibration drift worth catching. Explicit non-goals for v1.36: no new compliance item, no structural test, no count — the spec is already gathering more rails than is healthy and the right correction here is a calibration nudge, not another rail.
- **v1.35 (May 2026)** — Long-form variety rule + In Practice theme catalogue expansion. Issue 12 v1.32 shipped all three back-of-book reflective sections on coding agents (In Practice = Atlassian / Wu on 16% / agent-era practice; Outside In = Shopify natural-language spec layer for agentic workflows; Leadership Read = Brooker on coding agents as feedback loops). The v1.32 reader-experience AI count (≤4 of 6 featured) caught the issue overall but didn't catch that the three CURATED LONG-FORM PIECES — the load-bearing reflective content at the back of the issue — were all on the same topic. **New v1.35 rule: of the three long-form sections (In Practice / Outside In / Leadership Read), at most ONE can have AI / agents as its primary subject.** The other two cover non-AI engineering leadership: management craft, technical leadership for managers, culture & ways of working, org structure, production / operating at scale, career & coaching. **Deep Dives are exempt** — by design they converge on one topic. Plus the In Practice theme catalogue is expanded: technical leadership themes added (technical decisions when not the SME, architectural reviews as a manager, build-vs-buy as a leadership conversation, technical strategy under uncertainty); managing-across-and-upwards themes added (managing upwards / influencing senior leadership, escalation craft, presenting to non-technical exec audiences, cross-team partnership, handling reorgs and ambiguous mandates). The agent-era theme stays in the pool but doesn't dominate the run.
- **v1.34 (May 2026)** — Two small protections for In Practice season state, both prompted by the v1.32 test issue rendering *"Agent-era engineering practice — week 1 of 3"* in the theme label. (1) **State writes only on Deliver — made explicit.** The state file (`engineering-digest-state.json`) was already only being updated at the Deliver step in practice, but the protection lived in workflow-step ordering rather than as a stated rule. v1.34 adds an explicit paragraph at Step 7 making clear that test runs do not pollute season state — `current_in_practice_season`, `in_practice_history`, `last_issue_number`, and rotation fields all remain unchanged on un-published test runs. Operators can iterate test runs against the same week without season contamination. (2) **No-week-count compliance check added.** The v3.5 In Practice format rule already said *"theme name only, no week count"* — the rationale being that a visible *"week 1 of 3"* turns the existing "cut the season short rather than drag" rule into a broken promise to the reader. The rule was in the spec but not in the compliance checklist; Phase 4 violated it in the v1.32 test issue and Phase 5 lightweight didn't spot-check it. v1.34 adds an explicit checklist item with a grep verification (`.ip-theme` element must not contain *"week"*, *"of 3"*, *"part "*, etc.).
...
|---|---|---|---|---|
| Standard Weekly | 2–3+ stories from top-priority areas (default) | `assets/weekly-template.html` | 2,600–3,700 words | Default (cost-effective) |
...
| Light Edition | Top-priority areas are quiet | `assets/light-template.html` | 1,200–2,000 words | Default (cost-effective) |
...
**State writes only on Deliver (v1.34 — explicit).** The state file (`engineering-digest-state.json`) is updated **only** at this Step 7. Test runs that do not publish skip Deliver entirely, so `last_issue_number`, `current_in_practice_season`, `in_practice_season_progress`, `in_practice_history`, `outside_in_history`, `leadership_read_history`, and all rotation fields remain unchanged. This makes test runs safe to repeat without polluting season state — a test that labels itself *"Issue 12 / Agent-era engineering practice / week 1 of 3"* leaves zero footprint in state when not published, and the next real run starts from whatever the last published issue set. Operators running test iterations against the same week can do so freely without rotation contamination.
