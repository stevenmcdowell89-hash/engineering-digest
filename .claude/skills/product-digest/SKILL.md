...
- **Competitor set, by name.** The reader's direct competitive set rarely surfaces in general search — query each by name: Cardlytics, Reward (rewardinsight), Rezolve Ai (promoted at Issue 3 — Reward's owner since Feb 2026), Fidel API, Triple, Snoop, Bink; the networks' own offers/personalisation stacks (Mastercard personalised offers / Dynamic Yield, Visa offers products, Amex Offers); and bank in-house rewards builds. A quiet month for a named competitor is a valid finding to record; not querying them is not.
