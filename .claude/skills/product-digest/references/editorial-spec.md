...
- **Competitor set (query each by name, every issue):** Cardlytics · Reward (rewardinsight) · Rezolve Ai *(promoted at Issue 3 — owner of Reward since Feb 2026; seen Issue 2 Mashreq, Issue 3 Zilch)* · Fidel API · Triple · Snoop · Bink · Mastercard personalised offers / Dynamic Yield · Visa offers products · Amex Offers · bank in-house rewards builds. These are who the reader is displaced by or compared against in an RFP; a Cardlytics platform move matters more to this reader than most network announcements.
- **UK bank set (query each by name, every issue — v1.3):** Lloyds Banking Group (Lloyds, Halifax, Bank of Scotland) · NatWest Group · Barclays · HSBC & First Direct · Santander UK · Nationwide & Virgin Money · TSB · Monzo · Starling · Revolut · Chase UK — against rewards / loyalty / offers / cashback / hub terms, plus the major groups' own newsroom pages. Banks are the buyer **and** the in-house competitor; a bank shipping a first-party offers hub (the Issue 1 Lloyds Rewards miss) is simultaneously competitive intelligence and a sales signal. A bank named inside one ad-hoc query is not covered.
- **Patterned catch-alls (v1.4 — the recall layer):** name-agnostic queries run every issue: "UK bank rewards launch <month>", "UK bank loyalty programme launch <month>", "challenger bank cashback rewards <month>", "building society rewards <month>", plus per-region equivalents. **Coverage is layered by design: names for precision on the actors where a miss is most expensive; shapes for recall on everyone a list can't anticipate; outlets for what neither ranks in general search; the promotion loop to convert recurring shape-hits into names.** No single layer is trusted to be complete.
...
**The source map is self-correcting (v1.2).** The named lists above are a June 2026 snapshot; left alone they would go stale while creating false confidence (querying the list is not covering the market). Three rules keep the lists honest using the pipeline's own output (they apply to the competitor list **and** the UK bank list):
